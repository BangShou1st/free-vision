...
def _load_opencode_api_key() -> str:
    try:
        ...
        raise GatewayError(
            "missing_api_key",
            "Free Vision OpenCode API key is not configured for the ZCode gateway.",
            status=401,
        ) from exc
    ...
    *,
    ...
    if host == "opencode.ai":
        ...
        if not api_key:
            raise GatewayError(
                "missing_api_key",
                "Free Vision OpenCode API key is empty.",
                status=401,
            )
        ...
        try:
            ...
            result["User-Agent"] = os.environ.get(
                "ZEN_USER_AGENT", DEFAULT_ZEN_USER_AGENT
            )
        except ImportError:
            result["User-Agent"] = os.environ.get(
                "ZEN_USER_AGENT", "opencode/1.18.16"
            )
    return result
...
def _handler_factory(
    ...
    analyzer: Callable[[list[str], str], VisionResult],
    ...
    class GatewayHandler(BaseHTTPRequestHandler):
        ...
        def log_message(self, *_ignored: Any) -> None:
            return
        ...
        def _relay_response(
            ...
            *,
            ...
            stream_response = preloaded_body is None and (
                is_stream or content_type.startswith("text/event-stream")
            )
            ...
            response_body = (
                preloaded_body if preloaded_body is not None else response.read()
            )
            for key, value in response_headers:
                self.send_header(key, value)
            ...
            *,
            ...
            try:
                ...
                self._json_error(
                    ...
                    "Unable to reach the configured ZCode upstream provider.",
                )
            finally:
                if conn is not None:
                    conn.close()
        ...
        def _transform(
            ...
            try:
                transformed, changed = transform_chat_request(
                    payload,
                    analyzer=analyzer,
        ...
        def _chat_with_adaptive_fallback(
            self,
            payload: dict[str, Any],
            original_body: bytes,
        ) -> None:
            ...
            try:
                ...
                if response.status >= 400:
                    ...
                    if _is_text_only_image_rejection(
...
def create_gateway_server(
    ...
    *,
    ...
    if host not in {"127.0.0.1", "localhost", "::1"}:
        raise GatewayError(
            "unsafe_bind",
            "ZCode gateway only binds to loopback addresses by default.",
            status=500,
        )
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise GatewayError(
            "invalid_upstream",
            "ZCode gateway upstream must be an absolute http:// or https:// URL.",
            status=500,
        )
    return ThreadingHTTPServer(
        (host, port),
        _handler_factory(upstream_base_url, analyzer, api_key_loader),
    )
