...
4. `free_vision.install` includes `source.json` in the runtime payload whenever the source tree carries it and prints the canonical source after installation. The canonical repository is regression-tested to always carry this file, while synthetic/minimal source trees used by compatibility tests remain installable.
...
Regression tests must verify `source.json` is part of the canonical repository install payload and copied into installed Skills, its version/repository values are correct, `SKILL.md` contains the canonical URL plus a no-search rule, and installer output exposes the canonical source.
